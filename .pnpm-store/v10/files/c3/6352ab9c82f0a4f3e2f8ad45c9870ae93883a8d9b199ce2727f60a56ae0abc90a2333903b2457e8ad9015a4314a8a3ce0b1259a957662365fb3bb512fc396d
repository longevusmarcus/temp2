import { PolarCore } from "../core.js";
import { RequestOptions } from "../lib/sdks.js";
import { CheckoutLink } from "../models/components/checkoutlink.js";
import { ConnectionError, InvalidRequestError, RequestAbortedError, RequestTimeoutError, UnexpectedClientError } from "../models/errors/httpclienterrors.js";
import { HTTPValidationError } from "../models/errors/httpvalidationerror.js";
import { ResourceNotFound } from "../models/errors/resourcenotfound.js";
import { SDKError } from "../models/errors/sdkerror.js";
import { SDKValidationError } from "../models/errors/sdkvalidationerror.js";
import { CheckoutLinksUpdateRequest } from "../models/operations/checkoutlinksupdate.js";
import { APIPromise } from "../types/async.js";
import { Result } from "../types/fp.js";
/**
 * Update Checkout Link
 *
 * @remarks
 * Update a checkout link.
 *
 * **Scopes**: `checkout_links:write`
 */
export declare function checkoutLinksUpdate(client: PolarCore, request: CheckoutLinksUpdateRequest, options?: RequestOptions): APIPromise<Result<CheckoutLink, ResourceNotFound | HTTPValidationError | SDKError | SDKValidationError | UnexpectedClientError | InvalidRequestError | RequestAbortedError | RequestTimeoutError | ConnectionError>>;
//# sourceMappingURL=checkoutLinksUpdate.d.ts.map