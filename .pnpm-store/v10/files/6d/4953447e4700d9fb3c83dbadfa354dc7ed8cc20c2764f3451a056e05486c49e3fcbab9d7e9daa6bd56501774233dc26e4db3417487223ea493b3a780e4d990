...
exports.metersEvents = metersEvents;
...
const metersevents_js_1 = require("../models/operations/metersevents.js");
 ...
 */
function metersEvents(client, request, options) {
    return new async_js_1.APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => metersevents_js_1.MetersEventsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const path = (0, url_js_1.pathToFunc)("/v1/meters/{id}/events")(pathParams);
    ...
    const [result, raw] = await M.match(M.json(200, metersevents_js_1.MetersEventsResponse$inboundSchema, { key: "Result" }), M.jsonErr(404, resourcenotfound_js_1.ResourceNotFound$inboundSchema), M.jsonErr(422, httpvalidationerror_js_1.HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
