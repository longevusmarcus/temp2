...
const oauth2clientslist_js_1 = require("../models/operations/oauth2clientslist.js");
 ...
 * List OAuth2 clients.
 */
function oauth2ClientsList(client, request, options) {
    return new async_js_1.APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => oauth2clientslist_js_1.Oauth2ClientsListRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, oauth2clientslist_js_1.Oauth2ClientsListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, httpvalidationerror_js_1.HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
