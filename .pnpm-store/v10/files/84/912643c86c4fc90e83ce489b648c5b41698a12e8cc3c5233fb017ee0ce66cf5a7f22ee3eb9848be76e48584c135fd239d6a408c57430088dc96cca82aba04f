...
async function $do(client, security, request, options) {
    ...
    const [result, raw] = await M.match(M.json(200, customerportallicensekeyslist_js_1.CustomerPortalLicenseKeysListResponse$inboundSchema, {
        key: "Result",
    }), M.jsonErr(401, unauthorized_js_1.Unauthorized$inboundSchema), M.jsonErr(404, resourcenotfound_js_1.ResourceNotFound$inboundSchema), M.jsonErr(422, httpvalidationerror_js_1.HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
