...
async function $do(client, request, options) {
    ...
    const [result] = await M.match(M.json(200, Customer$inboundSchema), M.jsonErr(404, ResourceNotFound$inboundSchema), M.jsonErr(422, HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
